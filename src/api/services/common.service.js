...
const searchArea = async (area, pageNumber, latitude, longitude) => {
    ...
    // DOCS: x가 경도, y가 위도!
    const url = `https://dapiasfd.kakao.com/v2/local/search/keyword.json?sort=${SORT_METHOD}&page=${pageNumber}&size=${DATA_COUNT}&x=${longitude}&y=${latitude}&query=${encodeURIComponent(
        area
    )}`;
